...
describe('dashboard hostile data semantics', () => {
  ...
  it('marks a future schema missing a required column as incompatible without migrating', () => {
    ...
    try {
      writable.exec(
        "INSERT INTO schema_migrations (version, name, applied_at) VALUES (10, '010_future', '2026-08-17T00:00:00.000Z')",
      );
    ...
    expect(snapshot.database.data?.schemaVersion).toBe(10);
