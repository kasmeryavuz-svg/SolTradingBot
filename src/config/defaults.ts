export const DEFAULT_NODE_ENV = 'development';
export const DEFAULT_LOG_LEVEL = 'info';
export const DEFAULT_TRADING_ENABLED = false;
export const DEFAULT_LIVE_BROADCAST_ENABLED = false;
export const DEFAULT_SOLANA_NETWORK = 'mainnet-beta';
export const DEFAULT_SOLANA_RPC_TIMEOUT_MS = 10_000;
export const DEFAULT_SOLANA_RPC_URL = 'https://api.mainnet-beta.solana.com';

export const WRAPPED_SOL_MINT = 'So11111111111111111111111111111111111111112';
export const USDC_MINT = 'EPjFWdd5AufqSSqeM2qN1xzybapC8G4wEGGkZwyTDt1v';
export const DEFAULT_MARKET_DATA_TOKEN_MINTS = [WRAPPED_SOL_MINT, USDC_MINT] as const;
export const DEFAULT_MARKET_DATA_TIMEOUT_MS = 10_000;
export const DEFAULT_MARKET_DATA_POLL_INTERVAL_MS = 15_000;

export const DEFAULT_DISCOVERY_ENABLED = true;
export const DEFAULT_DISCOVERY_INCLUDE_PROFILES = true;
export const DEFAULT_DISCOVERY_INCLUDE_BOOSTS = true;
export const DEFAULT_DISCOVERY_TIMEOUT_MS = 10_000;
export const DEFAULT_DISCOVERY_POLL_INTERVAL_MS = 30_000;
export const DEFAULT_DISCOVERY_MAX_CANDIDATES = 20;
export const DEFAULT_DISCOVERY_ENRICH_MARKET_DATA = true;
export const DISCOVERY_MAX_CANDIDATES_LIMIT = 100;

export const DEFAULT_DATABASE_ENABLED = true;
export const DEFAULT_DATABASE_PATH = './data/soltradingbot.sqlite';
export const MEMORY_DATABASE_PATH = ':memory:';
export const DEFAULT_DATABASE_BUSY_TIMEOUT_MS = 5_000;
export const DEFAULT_HISTORY_LIMIT = 20;
export const HISTORY_LIMIT_MAX = 100;

export const DEFAULT_RISK_SCAN_TIMEOUT_MS = 10_000;
export const DEFAULT_RISK_SCAN_COMMITMENT = 'confirmed';
export const DEFAULT_RISK_HISTORY_LIMIT = 20;
export const DEFAULT_FEATURE_HISTORY_LIMIT = 20;
export const DEFAULT_STRATEGY_HISTORY_LIMIT = 20;
export const DEFAULT_PAPER_HISTORY_LIMIT = 20;
export const DEFAULT_POSITION_HISTORY_LIMIT = 20;
export const DEFAULT_EXIT_HISTORY_LIMIT = 20;
export const DEFAULT_PERFORMANCE_TRADE_LIMIT = 20;
export const DEFAULT_RESEARCH_TRADE_LIMIT = 20;
export const DEFAULT_DASHBOARD_PORT = 4313;
export const DASHBOARD_PORT_MIN = 1024;
export const DASHBOARD_PORT_MAX = 65535;

export const DEFAULT_EXECUTION_PROVIDER_TIMEOUT_MS = 5_000;
export const EXECUTION_PROVIDER_TIMEOUT_MS_MIN = 500;
export const EXECUTION_PROVIDER_TIMEOUT_MS_MAX = 15_000;
