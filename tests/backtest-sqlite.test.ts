...
describe('sqlite historical source', () => {
  ...
  it('accepts schema 7 and does not write rows during a backtest run', () => {
    const path = tempDbPath();
    const before = seedHistoricalDb(path);
    expect(before.schemaMigrations).toBe(LATEST_SCHEMA_VERSION);
    expect(COMPATIBLE_SCHEMA_VERSIONS).toEqual([4, 5, 6, 7]);
    ...
    expect(afterRepo.getStats().schemaVersion).toBe(7);
  });
  ...
  it('accepts schema 6 and does not migrate it', () => {
    ...
    try {
      expect(after.prepare('SELECT MAX(version) AS version FROM schema_migrations').get()?.['version']).toBe(6);
      expect(
        after.prepare("SELECT name FROM sqlite_master WHERE type = 'table' AND name = 'exit_evaluations'").get(),
      ).toBeUndefined();
    } finally {
      after.close();
    }
