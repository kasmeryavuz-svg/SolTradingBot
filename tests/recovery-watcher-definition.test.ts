...
describe('rw0_v1 frozen definitions', () => {
  it('freezes spec names and deterministic fingerprints', () => {
    expect(RW0_SPEC_VERSION).toBe('rw0_v1');
    ...
    expect(fingerprintRecoveryV0Signal(canonicalRecoveryV0Signal())).toBe(RECOVERY_V0_SIGNAL_FINGERPRINT);
  ...
  it('binds the recovery migration digest into the watcher fingerprint', () => {
    expect(canonicalRecoveryWatcherDefinition().recoveryMigrationSqlDigest).toBe(recoveryMigrationSqlDigest(1));
    expect(
      fingerprintRecoveryWatcherDefinition(
  ...
  it('changes the signal fingerprint when a frozen threshold changes', () => {
    expect(
      fingerprintRecoveryV0Signal(
        mutateCanonicalRecoveryV0Signal((definition) => {
          definition.recoveryConfirmation.liquidityUsd.minInclusive = RECOVERY_V0_MIN_CONFIRMATION_LIQUIDITY_USD + 1;
        }),
      ),
    ...
    expect(
      fingerprintRecoveryV0Signal(
        mutateCanonicalRecoveryV0Signal((definition) => {
          definition.dipObservation.volume5mUsd.minInclusive = RECOVERY_V0_MIN_DIP_VOLUME_5M_USD + 1;
        }),
      ),
    ).not.toBe(RECOVERY_V0_SIGNAL_FINGERPRINT);
    expect(
      fingerprintRecoveryV0Signal(
        mutateCanonicalRecoveryV0Signal((definition) => {
          definition.dipObservation.priceChange5mPct.minInclusive = RECOVERY_V0_MIN_PRICE_CHANGE_5M_PCT - 1;
        }),
      ),
    ).not.toBe(RECOVERY_V0_SIGNAL_FINGERPRINT);
    expect(
      fingerprintRecoveryV0Signal(
        mutateCanonicalRecoveryV0Signal((definition) => {
          definition.dipObservation.priceChange5mPct.maxInclusive = RECOVERY_V0_MAX_PRICE_CHANGE_5M_PCT + 1;
        }),
      ),
    ).not.toBe(RECOVERY_V0_SIGNAL_FINGERPRINT);
    expect(
      fingerprintRecoveryV0Signal(
        mutateCanonicalRecoveryV0Signal((definition) => {
          definition.recoveryConfirmation.volumeToLiquidity5m.minInclusive = RECOVERY_V0_MIN_VOLUME_TO_LIQUIDITY_5M + 0.1;
        }),
      ),
    ).not.toBe(RECOVERY_V0_SIGNAL_FINGERPRINT);
    expect(
  ...
  it('binds unproven signal, distinct shadow/paper tracks, unreachable safe paper, and unresolved safety gates', () => {
    ...
    expect(definition.signal.historicalSampleCadence).toBe('sparse_approximately_5_minute_observations');
    ...
    expect(definition.holderGate).toBe('UNKNOWN_unimplemented');
    expect(definition.bundleGate).toBe('UNKNOWN_unimplemented');
    expect(definition.discoveryCoverageComplete).toBe(false);
    expect(definition.largestRealHolderPctImplemented).toBe(false);
    expect(definition.linkedBundlePctImplemented).toBe(false);
    expect(definition.unexplainedTop20RemainderDoesNotProveHiddenSingleAccountOverTenPercent).toBe(true);
    expect(definition.safePaperEntry.unimplementedInRw0V1).toBe(true);
    expect(definition.safePaperEntry.neverBackfillRecoveryConfirmationPriceIfSafetyCompletedLater).toBe(true);
    ...
    expect(definition.signal.recoveryConfirmation.legalOnlyWhenRecoveryConfirmedAtStrictlyBeforeWatchExpiresAt).toBe(
      true,
    );
    expect(definition.signal.recoveryConfirmation.exactWatchExpiryBoundaryBelongsToExpired).toBe(true);
    ...
    expect(definition.operational.holderBundleCreatorEvidenceUnknownOnlyInRw0V1).toBe(true);
    ...
    expect(definition.exit.intendedComparatorWhenImplemented.closedRequiresObservedExitEvidence).toBe(true);
    expect(definition.exit.intendedComparatorWhenImplemented.observedAtAndObservationCollectedAtAreTheSameInstant).toBe(
      true,
    );
