...
export function loadCoreConfig(source: EnvSource): CoreAppConfig {
  return {
...
function loadDiscoveryConfig(source: EnvSource): CoreAppConfig['discovery'] {
  const discovery = {
...
function loadDatabaseConfig(source: EnvSource): CoreAppConfig['database'] {
  return {
    enabled: parseBooleanFlag(
...
function loadRiskConfig(source: EnvSource): CoreAppConfig['risk'] {
  return {
    timeoutMs: parsePositiveInteger(
...
function loadFeatureConfig(source: EnvSource): CoreAppConfig['features'] {
  return {
    historyLimit: parseBoundedPositiveInteger(
...
function loadStrategyConfig(source: EnvSource): CoreAppConfig['strategy'] {
  return {
    historyLimit: parseBoundedPositiveInteger(
...
function loadPaperConfig(source: EnvSource): CoreAppConfig['paper'] {
  return {
    historyLimit: parseBoundedPositiveInteger(
...
function loadPositionConfig(source: EnvSource): CoreAppConfig['position'] {
  return {
    historyLimit: parseBoundedPositiveInteger(
      readOptionalEnv(source, 'POSITION_HISTORY_LIMIT'),
      DEFAULT_POSITION_HISTORY_LIMIT,
...
function loadExitConfig(source: EnvSource): CoreAppConfig['exit'] {
  return {
    historyLimit: parseBoundedPositiveInteger(
      readOptionalEnv(source, 'EXIT_HISTORY_LIMIT'),
      DEFAULT_EXIT_HISTORY_LIMIT,
      'EXIT_HISTORY_LIMIT',
      HISTORY_LIMIT_MAX,
    ),
...
function loadPerformanceConfig(source: EnvSource): CoreAppConfig['performance'] {
  return {
...
function loadResearchConfig(source: EnvSource): CoreAppConfig['research'] {
  return {
    tradeLimit: parseBoundedPositiveInteger(
...
function loadDashboardConfig(source: EnvSource): CoreAppConfig['dashboard'] {
  return {
