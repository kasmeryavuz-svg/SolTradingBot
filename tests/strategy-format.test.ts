...
import { evaluatePassing, passingVector, withAvailableNumber, withUnavailable } from './strategy-fixtures.js';
...
describe('strategy formatter', () => {
  it('prints experimental classification language and never treats ENTRY_CANDIDATE as an order', () => {
    ...
    expect(lines).toContain('ENTRY_CANDIDATE is a strategy classification only. No order or trade is created.');
    expect(lines).toContain('Context — NOT s07_v1 decision rules');
    expect(lines).toContain('Checkpoint: 08');
    expect(lines).not.toMatch(/profitable strategy|winning strategy|high-win-rate|optimized|validated edge|proven alpha/i);
  ...
  it('explains NO_ENTRY as a failed rule, not a sell', () => {
    ...
    const lines = formatStrategyCheckLines(vector, evaluateStrategy(vector, { evaluatedAt: T_10_00 })).join('\n');
