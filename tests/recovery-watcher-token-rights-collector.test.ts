...
describe('recovery token-rights collector', () => {
  ...
  it('preserves active authority and evaluates FAIL', async () => {
    const result = await collectTokenRights({
  ...
  it('returns unavailable for provider outage and keeps malformed responses fatal', async () => {
    const outage = await collectTokenRights({
      provider: {
        getMintAccount: vi.fn(() => Promise.reject(new Error('RPC unavailable'))),
      },
      tokenMint: MINT,
      now,
    });
    ...
    await expect(
      collectTokenRights({ provider: provider(null), tokenMint: MINT, now }),
